...
| Deep or very large directory trees | Depth and entry limits, reported as diagnostics rather than silently truncating. Import refuses a truncated scan (`STEMMA1303`) unless `--allow-incomplete-scan` is given, so a partial import is always an explicit decision |
