...
| Oversized configuration | Per-file (2 MiB), total (64 MiB), directory depth (32), registered configuration files (20 000) and inspected directory entries (1 000 000) limits, plus front matter limits: 64 KiB, 2 000 lines, 8 levels of nesting, 500 keys |
| Deep, very large or partly unreadable directory trees | Depth and entry limits and unreadable directories (`STEMMA1305`) are reported as diagnostics rather than silently truncating. Import refuses an incomplete scan (`STEMMA1303`) unless `--allow-incomplete-scan` is given, so a partial import is always an explicit decision |
